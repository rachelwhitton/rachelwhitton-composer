...
# Use custom domains
export CUSTOM_DOMAIN=( $(terminus domain:list $TERMINUS_SITE.$TERMINUS_ENV --field=id) )
export BEHAT_PARAMS='{"extensions":{"Behat\\MinkExtension":{"base_url":"https://'${CUSTOM_DOMAIN[-1]}'"},"PaulGibbs\\WordpressBehatExtension":{"site_url":"https://'${CUSTOM_DOMAIN[-1]}'/wp","users":{"admin":{"username":"'$ADMIN_USERNAME'","password":"'$ADMIN_PASSWORD'"}},"wpcli":{"binary":"terminus -n wp '$TERMINUS_SITE'.'$TERMINUS_ENV' --"}}}}'
